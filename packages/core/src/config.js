...
export const mt5MarketRegionsByInstrumentSchema = z.record(z.string().trim().min(1), z.array(z.string().trim().min(1)).min(1));
...
export function validateStrategyConfig(raw) {
    ...
    if (policySchema) {
        return {
            ...config,
            policy: policySchema.parse(config.policy),
        };
    }
    return config;
}
export const ALPACA_OPTIONS_POLICY_DEFAULTS = {
    dryRun: true,
    model: "",
    maxLossPerPlay: 500,
};
...
export const POLYMARKET_POLICY_DEFAULTS = {
    dryRun: true,
    model: "",
    maxBet: { mode: "fixed", value: 100 },
};
export const MT5_POLICY_DEFAULTS = {
    dryRun: true,
    model: "",
    maxRiskPercent: 2,
    minRiskReward: 0.5,
...
export const BINANCE_POLICY_DEFAULTS = {
    dryRun: true,
    model: "",
