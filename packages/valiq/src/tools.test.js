...
describe("createValiqBreakingNewsTool", () => {
    ...
    it("passes both parameters to the endpoint", async () => {
        ...
        const call = vi.mocked(mockClient.request).mock.calls[0][0];
    ...
    it("propagates API errors", async () => {
        vi.mocked(mockClient.request).mockRejectedValue(new Error("Val-iQ Data API error: 500 Internal Server Error"));
        const tool = createValiqBreakingNewsTool(adapter);
...
describe("createValiqDataTool", () => {
    ...
    it("requires ticker for equity endpoints", async () => {
        const tool = createValiqDataTool(adapter);
        await expect(tool.handler({ endpoint: "getEquityOverview" })).rejects.toThrow("ticker is required");
    });
    it("requires region for macro endpoints", async () => {
        const tool = createValiqDataTool(adapter);
        await expect(tool.handler({ endpoint: "getMacroEconomy" })).rejects.toThrow("region is required");
    });
...
describe("ValiqDataClient", () => {
    it("sends X-API-Key header", async () => {
        ...
        expect(fetchSpy).toHaveBeenCalledWith("https://data.example.com/breaking-news", expect.objectContaining({
