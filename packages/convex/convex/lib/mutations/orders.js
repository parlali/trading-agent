...
export const createRun = mutation({
    args: {
        ...
        trigger: v.optional(v.union(v.literal("cron"), v.literal("manual"), v.literal("callback"))),
    },
    handler: async (ctx, args) => {
        ...
        if (freshActiveRun) {
            throw new Error(`Strategy ${args.strategyId} already has an active run (${freshActiveRun._id})`);
        }
...
export const recoverStaleRunningRuns = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const runningRuns = (await ctx.db.query("strategy_runs").collect()).filter((run) => run.status === "running");
...
export const logTradeEvent = mutation({
    args: {
        ...
        runId: v.id("strategy_runs"),
        strategyId: v.id("strategies"),
        eventType: v.union(v.literal("intent"), v.literal("validation"), v.literal("submission"), v.literal("fill_update"), v.literal("filled"), v.literal("rejected"), v.literal("cancelled")),
        payload: v.string(),
    },
    handler: async (ctx, args) => {
        requireServiceToken(args.serviceToken);
...
export const upsertOrder = mutation({
    args: {
        ...
        venue: v.string(),
        instrument: v.string(),
        status: v.union(v.literal("pending"), v.literal("partially_filled"), v.literal("filled"), v.literal("rejected"), v.literal("cancelled"), v.literal("expired"), v.literal("timed_out")),
        action: v.union(v.literal("entry"), v.literal("adjustment"), v.literal("close"), v.literal("modify"), v.literal("cancel")),
        quantity: v.number(),
...
export const logOrderTransition = mutation({
    args: {
        ...
        strategyId: v.id("strategies"),
        sequence: v.number(),
        type: v.union(v.literal("submission"), v.literal("status_change"), v.literal("modify_attempt"), v.literal("cancel_attempt"), v.literal("timeout_decision"), v.literal("terminal")),
        status: v.union(v.literal("pending"), v.literal("partially_filled"), v.literal("filled"), v.literal("rejected"), v.literal("cancelled"), v.literal("expired"), v.literal("timed_out")),
        previousStatus: v.optional(v.union(v.literal("pending"), v.literal("partially_filled"), v.literal("filled"), v.literal("rejected"), v.literal("cancelled"), v.literal("expired"), v.literal("timed_out"))),
