...
const equityTimeRangeV = v.union(v.literal("24h"), v.literal("7d"), v.literal("30d"), v.literal("90d"), v.literal("all"));
export const getPortfolioFreshness = query({
    ...
    handler: async (ctx, args) => {
        ...
        const rows = await Promise.all(apps.map((app) => ctx.db
...
export const getPortfolioTradeHistory = query({
    ...
    handler: async (ctx, args) => {
        ...
        const events = rawEvents
            ...
            .slice(0, limit);
        const orderIds = Array.from(new Set(events.map(extractOrderId).filter(Boolean)));
        const orders = await Promise.all(orderIds.map((orderId) => ctx.db
            .query("orders")
...
export const getPortfolioEquitySeries = query({
    ...
    handler: async (ctx, args) => {
        ...
        const apps = args.app ? [args.app] : VENUE_APPS;
        const snapshotWindows = await Promise.all(apps.map((app) => Promise.all([
            ctx.db
        ...
        return {
            ...
            latest: series[series.length - 1] ?? (latestByApp.size > 0
                ? {
...
function buildFreshnessDto(app, row) {
...
function extractInstrument(payload, order) {
    return readString(payload?.intent, "instrument")
        ?? readString(payload, "instrument")
        ?? order?.instrument;
}
function extractAction(payload, order) {
...
function extractSide(payload, order) {
...
function resolveRangeStart(timeRange, end) {
...
function resolveSnapshotEquity(snapshot) {
