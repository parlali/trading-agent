...
export const getActiveOrders = query({
    ...
    handler: async (ctx, args) => {
        ...
        const pending = await ctx.db
            .query("orders")
            .withIndex("by_strategy_status", (q) => q.eq("strategyId", args.strategyId).eq("status", "pending"))
            .collect();
...
export const getTradeHistory = query({
    args: {
        app: v.optional(v.union(v.literal("alpaca-options"), v.literal("polymarket"), v.literal("mt5"), v.literal("binance-futures"))),
        strategyId: v.optional(v.id("strategies")),
        runId: v.optional(v.id("strategy_runs")),
        eventTypes: v.optional(v.array(v.union(v.literal("intent"), v.literal("validation"), v.literal("submission"), v.literal("fill_update"), v.literal("filled"), v.literal("rejected"), v.literal("cancelled")))),
    ...
    handler: async (ctx, args) => {
        ...
        if (args.runId) {
            ...
            const appStrategies = await ctx.db
                ...
                .collect();
            const perStrategy = await Promise.all(appStrategies.map((strategy) => ctx.db
                .query("trade_events")
