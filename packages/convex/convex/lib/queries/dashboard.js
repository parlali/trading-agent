...
export const getDashboardOverview = query({
    ...
    handler: async (ctx) => {
        ...
        const [systemState, appHealth, strategies, runs, alerts,] = await Promise.all([
            ctx.db
                .query("system_state")
        ...
        const [accountSnapshots, openPositionsByStrategy] = await Promise.all([
            Promise.all(VENUE_APPS.map((app) => ctx.db
        ...
        const latestRunByStrategy = new Map();
...
export const getPnlSummary = query({
    ...
    handler: async (ctx, args) => {
        ...
        const snapshotsByApp = await Promise.all(VENUE_APPS.map((app) => ctx.db
            .query("account_snapshots")
...
export const getEquityTimeSeries = query({
    args: {
        timeRange: v.union(v.literal("24h"), v.literal("7d"), v.literal("30d"), v.literal("90d"), v.literal("all")),
    },
    handler: async (ctx, args) => {
        await requireUser(ctx);
        const durationMsByRange = {
...
export const getAccountSnapshots = query({
    ...
    handler: async (ctx) => {
        await requireUser(ctx);
        const snapshots = await Promise.all(VENUE_APPS.map((app) => ctx.db
            .query("account_snapshots")
            .withIndex("by_app_timestamp", (q) => q.eq("app", app))
            .order("desc")
            .first()));
