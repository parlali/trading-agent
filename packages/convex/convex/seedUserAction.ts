...
type SeedUserResult =
    ...
    | { status: "created"; email: string; userId: Id<"users"> }
...
export const seedUser = internalAction({
    ...
    handler: async (ctx, args): Promise<SeedUserResult> => {
        ...
        const userId: Id<"users"> = await ctx.runMutation(
            internal.seedUserHelpers.insertUser,
            { email: args.email }
        )

        await ctx.runMutation(
            internal.seedUserHelpers.insertAuthAccount,
            { userId, email: args.email, secret }
        )
