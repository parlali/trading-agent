...
import { BinanceApiError, } from "./binance-client";
export class BinanceVenueAdapter {
    client;
    symbolRulesCache = new Map();
    ...
    async submitOrder(intent) {
        ...
        if (quantity * notionalPrice < rules.minNotional) {
            throw createExecutionError("pre_validation", `Order notional ${quantity * notionalPrice} is below minimum ${rules.minNotional} for ${symbol}`, {
    ...
    async getCurrentMarkPrice(symbol) {
    ...
    async getMarketSnapshot(symbols) {
        ...
        const snapshots = await Promise.all(normalized.map(async (symbol) => {
            const [bookTicker, premium] = await Promise.all([
    ...
    async normalizePrice(symbol, price) {
        const rules = await this.getSymbolRules(symbol.toUpperCase());
    ...
    async updateProtectionOrders(config) {
        ...
        const protectionOrders = existingOrders.filter((order) => order.reduceOnly && (order.type === "STOP_MARKET" ||
    ...
    async ensureTradingMode(symbol, leverage) {
...
function mapExecutionResult(symbol, order) {
...
function resolveBinanceVerificationPrice(intent) {
    if (intent.orderType === "limit" || intent.orderType === "stop_limit") {
        return intent.limitPrice;
    }
    return undefined;
}
function mapDepthToOrderBook(symbol, depth) {
    return {
        symbol,
...
function mapBinanceOrderStatus(status, filledQuantity, quantity) {
    if (status === "NEW" && filledQuantity > 0 && filledQuantity < quantity) {
        return "partially_filled";
    }
    return mapOrderStatus(status);
}
function mapToBinanceOrderType(orderType) {
    switch (orderType) {
        case "market":
            return "MARKET";
        case "limit":
...
function mapFromBinanceOrderType(orderType) {
    switch (orderType) {
        case "MARKET":
            return "market";
        case "LIMIT":
...
function mapTimeInForce(timeInForce) {
    switch (timeInForce) {
        case "ioc":
            return "IOC";
        case "fok":
...
function shouldSendTimeInForce(orderType) {
    return orderType === "LIMIT" || orderType === "STOP" || orderType === "TAKE_PROFIT";
}
function parseSymbolRules(symbol) {
...
function parseCompositeOrderId(orderId) {
...
function toCompositeOrderId(symbol, orderId) {
...
function resolveLeverage(intent) {
