...
export class AlpacaOptionsVenueAdapter {
    client;
    constructor(client) {
        this.client = client;
    }
    async getOptionsChain(underlyingSymbol, params = {}) {
        const contractsResponse = await this.client.getOptionContracts({
            underlyingSymbol,
            ...params,
        });
        const snapshotsResponse = await this.client.getOptionSnapshotsByUnderlying(underlyingSymbol, params);
        return {
    ...
    async buildCloseIntent(instrument) {
        const rawPositions = await this.client.getPositions();
    ...
    async closePosition(instrument, preparedIntent) {
    ...
    async verify(intent) {
        ...
        const knownContracts = new Set(contractsResponse.contracts.map((contract) => contract.symbol.toUpperCase()));
...
function groupIronCondorPositions(positions) {
...
function isIronCondorGroup(positions) {
...
function mapGroupedPosition(group) {
    return {
        instrument: group.instrument,
        side: "short",
        quantity: group.quantity,
...
function mapSinglePosition(position) {
    const parsed = parseOptionContractSymbol(position.symbol);
    return {
        instrument: position.symbol,
        side: position.side,
...
function mapWorkingOrder(order) {
...
function resolveOrderInstrument(order) {
...
function resolveGroupForClose(positions, instrument) {
...
function computeAlpacaStructurePrices(legs) {
    if (legs.length === 0) {
        return {};
    }
...
function mapAlpacaOrderStatus(status) {
    switch (status) {
        case "filled":
            return "filled";
