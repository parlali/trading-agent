...
describe("AlpacaClient host routing", () => {
    ...
    it("uses the trading host for option contract discovery", async () => {
        fetchMock.mockResolvedValue(new Response(JSON.stringify({ option_contracts: [] }), {
            status: 200,
            headers: {
        ...
        expect(String(fetchMock.mock.calls[0]?.[0])).toContain("https://paper-api.alpaca.markets/v2/options/contracts?");
    });
    it("uses the market-data host for equity quotes", async () => {
        ...
        expect(String(fetchMock.mock.calls[0]?.[0])).toContain("https://data.alpaca.markets/v2/stocks/SPY/quotes/latest");
    });
});
