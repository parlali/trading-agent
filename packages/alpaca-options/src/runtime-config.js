...
const ALPACA_ENVIRONMENT_HOSTS = {
    paper: {
...
export function resolveAlpacaCredentials(secrets) {
    return {
...
export function resolveAlpacaEnvironment(baseUrl) {
    const normalized = baseUrl.trim().toLowerCase();
    if (normalized === "paper" || normalized === "live") {
        return normalized;
    }
    throw new Error(`Invalid Alpaca environment "${baseUrl}". Set ALPACA_PRIMARY_ENVIRONMENT to "paper" or "live".`);
}
export function resolveAlpacaTradingBaseUrl(environment) {
    return ALPACA_ENVIRONMENT_HOSTS[environment].tradingBaseUrl;
}
export function resolveAlpacaMarketDataBaseUrl(environment) {
    return ALPACA_ENVIRONMENT_HOSTS[environment].marketDataBaseUrl;
}
export function resolveAlpacaRuntimeConfig(secrets) {
    const environment = resolveAlpacaEnvironment(requireResolvedSecret(secrets, "ALPACA_PRIMARY_ENVIRONMENT"));
    return {
        environment,
