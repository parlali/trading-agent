...
export class PolymarketVenueAdapter {
    ...
    async getMarketPrice(tokenId, side) {
    ...
    async getPositions() {
        ...
        const results = await Promise.all(tokenArray.map(async (tokenId) => {
    ...
    async submitOrder(intent) {
        ...
        const response = await this.client.createOrder({
            ...
            expiration: intent.metadata?.expiration,
            negRisk: intent.metadata?.negRisk,
        });
        return {
    ...
    async modifyOrder(orderId, changes) {
        ...
        const newIntent = {
            ...
            side: existing.side.toLowerCase(),
    ...
    async buildMarketSearchResult(market) {
        const tokens = await Promise.all(market.tokens.map(async (token) => {
            try {
...
// Helpers
// ---------------------------------------------------------------------------
function calculateAvgEntryPrice(trades) {
...
function mapPostOrderStatus(status) {
    switch (status) {
        case "matched":
            return "filled";
        case "live":
...
function mapOpenOrderStatus(order) {
...
function mapOpenOrderToExecutionResult(order) {
    const sizeMatched = Number(order.size_matched);
