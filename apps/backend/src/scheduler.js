import { ToolPool, ToolRegistry, createAlpacaGetOptionsChainTool, createAlpacaGetQuoteTool, createBinanceGetMarketPriceTool, createBinanceGetOrderBookTool, createBinanceProposeAdjustmentTool, createBinanceProposeCloseTool, createBinanceProposeOrderTool, createCancelOrderTool, createGetAccountTool, createGetOrderStatusTool, createGetPositionsTool, getToolCategory, createMT5ProposeCloseTool, createMT5GetSymbolInfoTool, createMT5ModifyOrderTool, createModifyOrderTool, createProposeAdjustmentTool, createPolymarketGetMarketPriceTool, createPolymarketGetOrderBookTool, createMT5ProposeAdjustmentTool, createMT5ProposeOrderTool, createPolymarketSearchMarketsTool, createPolymarketProposeCloseTool, createPolymarketProposeAdjustmentTool, createPolymarketProposeOrderTool, createProposeCloseTool, createProposeOrderTool, createWaitForOrderUpdateTool, createWebFetchTool, createWebSearchTool, executeAgentRun, withCallBudget, } from "@valiq-trading/agent";
import { createConvexOrderPersistenceAdapter } from "@valiq-trading/convex";
import { ExecutionPipeline, VENUE_APPS, isTerminalOrderStatus, createInstrumentConflictValidator, createKillSwitchGuardedVenue as createRuntimeKillSwitchGuardedVenue, filterPositionsByOwnership, binancePolicySchema, getNextCronFireMs, mt5PolicySchema, parseSummaryMetadata, stripMetadataBlock, validatePolicy, withTimeout, } from "@valiq-trading/core";
...
export const pendingManualTriggers = new Set();
import { backend, convexUrl, backendServiceToken, logger, plugins, resolvedSecrets, searchProvider, syncStrategies, killSwitchCheckers, healthState, } from "./state";
...
async function checkKillSwitch(app, context) {
    const checker = killSwitchCheckers[app];
    if (!checker)
        return false;
    return await checker(context);
}
function createKillSwitchGuardedVenue(venue, app, strategyId) {
    const checker = killSwitchCheckers[app];
    if (!checker)
        return venue;
    return createRuntimeKillSwitchGuardedVenue(venue, strategyId, checker);
}
function mergeRuntimeContextLines(existing, additional) {
    if (additional.length === 0) {
        return existing;
    }
    return [...(existing ?? []), ...additional];
}
function buildPendingOrderContext(snapshot) {
    return {
...
function getPendingOrderRecommendedAction(snapshot) {
...
function resolveExtraToolCategory(tool, runLogger, app) {
...
function logVenueToolMismatch(runLogger, app, toolName, expectedAdapter, venue) {
...
function registerCanonicalTool(toolPool, registration) {
...
function buildToolPool(config) {
    const { app, venue, pipeline, policy, extraTools, isCallback, runLogger, } = config;
    ...
    registerCanonicalTool(toolPool, {
        ...
        create: () => {
            ...
            if (app === "binance-futures") {
                ...
                return createBinanceProposeOrderTool(pipeline, venue, binancePolicySchema.parse(policy));
            }
            return null;
        },
    });
    ...
    registerCanonicalTool(toolPool, {
        name: "web_search",
        compatibleVenues: ["polymarket"],
        create: () => withCallBudget(createWebSearchTool(searchProvider), isCallback ? 2 : 5),
    });
    registerCanonicalTool(toolPool, {
        name: "web_fetch",
        compatibleVenues: ["polymarket"],
        create: () => withCallBudget(createWebFetchTool(), isCallback ? 1 : 3),
    });
    return toolPool;
}
async function reconcilePendingOrdersForRun(pipeline, strategyId, orderPersistence, runLogger) {
    ...
    for (const persistedOrder of persistedActiveOrders) {
        try {
            ...
            runtimeContextLines.push(`Active order refresh failed at run start for ${persistedOrder.orderId}. Do not trust the stored snapshot without a successful venue refresh.`);
            continue;
        }
...
export async function registerStrategyWithScheduler(scheduler, app, strategy) {
    ...
    const additionalSecrets = additionalSecretKeys.length > 0
    ...
    syncStrategies[app] ??= [];
    const alreadyTracked = syncStrategies[app].some((e) => e.strategy._id === strategy._id);
    if (!alreadyTracked) {
...
export async function runStrategy(app, plugin, strategy, policy, strategySecrets, scheduler, trigger = "cron") {
    ...
    let runtimeContextLines;
    if (plugin.preRunHooks) {
        const hookResult = await withTimeout(async () => await plugin.preRunHooks({
            venue,
            policy,
            strategyId: strategy._id,
            logger,
            createAlert: (alert) => backend.createAlert(alert),
        }), PRE_RUN_HOOK_TIMEOUT_MS, `pre-run hooks for strategy ${strategy._id}`);
        if (hookResult.skip) {
    ...
    const budgetedExtraTools = extraTools.map((tool) => isCallback ? withCallBudget(tool, 2) : tool);
    ...
    try {
        await withTimeout(async () => {
            ...
            const { pendingOrders, runtimeContextLines: pendingOrderRuntimeContext } = await reconcilePendingOrdersForRun(pipeline, strategy._id, orderPersistence, runLogger);
            ...
            if (plugin.postRunHooks) {
                await withTimeout(async () => await plugin.postRunHooks({
                    venue,
                    policy,
                    strategyId: strategy._id,
                    logger: runLogger,
                    createAlert: (alert) => backend.createAlert(alert),
                }), POST_RUN_HOOK_TIMEOUT_MS, `post-run hooks for strategy ${strategy._id}`);
            }
            if (isDryRun) {
            ...
            if (scheduler && result.summary) {
                ...
                if (metadata?.nextRunInMinutes) {
                    ...
                    if (nextCronMs && delayMs >= nextCronMs) {
                        ...
                        void backend.recordRunCallback(runId, metadata.nextRunInMinutes, callbackFiresAt);
                    }
                }
            }
