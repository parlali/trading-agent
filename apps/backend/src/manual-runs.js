import { MANUAL_RUN_POLL_INTERVAL_MS, ALL_APPS, backend, logger, manualRunPollTimer, manualRunPollInFlight, setManualRunPollTimer, setManualRunPollInFlight, } from "./state";
import { registerStrategyWithScheduler, pendingManualTriggers } from "./scheduler";
export function startManualRunPolling(scheduler) {
...
export function stopManualRunPolling() {
    if (manualRunPollTimer) {
        clearInterval(manualRunPollTimer);
        setManualRunPollTimer(null);
    }
}
export async function pollManualRunRequests(scheduler) {
