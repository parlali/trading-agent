import { createClient, resolveArg, printDeleteCounts, runScript, } from "./lib/strategy-cli";
...
runScript(async () => {
    ...
    if (idArg) {
        ...
        const match = all.find((s) => s.name.toLowerCase() === nameArg.toLowerCase());
        if (!match) {
            const available = all.map((s) => `  - ${s.name} (${s._id})`).join("\n");
            throw new Error(`Strategy "${nameArg}" not found in backend.\nExisting:\n${available}`);
        }
        targetId = match._id;
    ...
    printDeleteCounts(result.deleted);
});
