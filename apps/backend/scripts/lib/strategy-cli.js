...
export function resolveArg(name) {
    const prefix = `--${name}=`;
...
export async function loadStrategiesFromDocument() {
...
export function findStrategyByName(strategies, name) {
    const match = strategies.find((s) => s.name.toLowerCase() === name.toLowerCase());
    if (!match) {
        const available = strategies.map((s) => `  - ${s.name}`).join("\n");
        throw new Error(`Strategy "${name}" not found in document.\nAvailable:\n${available}`);
    }
    return match;
}
export function printDeleteCounts(deleted) {
