...
export function TradeHistoryTable({ trades, title, }) {
    if (trades.length === 0) {
        return (<EmptyState icon={List} title="No trade events" description="Trade events will appear here when strategies execute"/>);
    }
    return (<div className="space-y-3">
            ...
            <div className="sm:hidden">
                <CardList data={trades} getKey={(trade) => trade.eventId} renderCard={(row) => (<div className="rounded-lg border border-border-subtle p-3 space-y-2">
                            <div className="flex items-center justify-between">
                                ...
                                {row.price != null ? (<span className="text-sm font-mono tabular-nums shrink-0">
            ...
            <PortfolioSection className="hidden sm:block" title={title} count={title ? trades.length : undefined}>
                <DataTable columns={tradeHistoryColumns} data={trades} getRowKey={(trade) => trade.eventId}/>
            </PortfolioSection>
        </div>);
}
