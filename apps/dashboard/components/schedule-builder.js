...
function dayModeToCronDow(state) {
...
function parseDowToDayMode(dow) {
    if (dow === "*")
        return { mode: "every-day", days: [] };
    if (dow === "1-5")
        return { mode: "weekdays", days: [] };
    const dayNumbers = dow.split(",").map(Number);
    if (dayNumbers.some(isNaN))
        return null;
    ...
    if (dayIndices.length > 0)
        return { mode: "specific-days", days: dayIndices };
    return null;
}
export function ScheduleBuilder({ value, onChange }) {
    const [state, setState] = useState(() => cronToSchedule(value));
    const lastEmittedCron = useRef(value);
    const emitCron = useCallback((next) => {
    ...
    if (state.isCustom) {
        return (<div className="space-y-3">
                <div className="space-y-1.5">
                    ...
                    <Input placeholder="e.g. 0 14 * * 1-5" className="font-mono" value={state.customCron} onChange={(e) => {
                ...
                <Button type="button" variant="outline" size="sm" onClick={() => update({ isCustom: false })}>
    ...
    return (<div className="space-y-4">
            ...
            <div className="space-y-1.5">
                ...
                <Select value={state.runMode} onValueChange={(v) => update({ runMode: v })}>
            ...
            {state.runMode === "interval" ? (<div className="space-y-3">
                    <div className="flex items-center gap-2">
                        <Label className="text-sm whitespace-nowrap">Every</Label>
                        <Input type="number" min={1} max={state.intervalUnit === "minutes" ? 59 : 23} className="w-20" value={state.intervalValue} onChange={(e) => update({ intervalValue: Number(e.target.value) || 1 })}/>
                        <Select value={state.intervalUnit} onValueChange={(v) => update({ intervalUnit: v })}>
                    ...
                    <div className="flex items-center gap-3">
                        <Switch checked={state.hourWindowEnabled} onCheckedChange={(checked) => update({ hourWindowEnabled: checked })}/>
                        <Label className="text-sm">Restrict to specific hours</Label>
                    </div>

                    {state.hourWindowEnabled ? (<div className="flex items-center gap-2">
                            <Label className="text-xs text-muted-foreground whitespace-nowrap">From</Label>
                            <Select value={state.hourStart} onValueChange={(v) => update({ hourStart: v })}>
                            ...
                            <Select value={state.hourEnd} onValueChange={(v) => update({ hourEnd: v })}>
                                <SelectTrigger className="w-20">
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                    ...
                    <div className="space-y-1.5">
                        ...
                        <Select value={state.minute} onValueChange={(v) => update({ minute: v })}>
                            <SelectTrigger className="w-20">
                                <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
            ...
            <div className="space-y-1.5">
                ...
                <Select value={state.dayMode} onValueChange={(v) => update({ dayMode: v })}>
                    <SelectTrigger className="w-full">
                        <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
            ...
            {state.dayMode === "specific-days" ? (<div className="flex gap-1.5">
                    {DAY_LABELS.map((label, index) => (<Button key={label} type="button" size="sm" variant={state.days.includes(index) ? "default" : "outline"} className="h-8 w-10 px-0 text-xs" onClick={() => toggleDay(index)}>
                            {label}
                        </Button>))}
                </div>) : null}
            ...
            <div className="flex items-center justify-between">
                ...
                <Button type="button" variant="ghost" size="sm" className="text-xs text-muted-foreground h-auto py-1" onClick={() => update({ isCustom: true, customCron: scheduleToCron(state) })}>
                    Edit as cron
                </Button>
            </div>
        </div>);
