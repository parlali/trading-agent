...
function StrategyCard({ strategy, onRun }) {
    ...
    return (<div role="button" tabIndex={0} onClick={() => router.push(`/strategies/${strategy._id}`)} onKeyDown={(e) => {
            ...
            <div className="flex items-center gap-2 sm:gap-3 shrink-0 ml-2">
                {strategy.latestRun ? (<div className="text-right hidden sm:block">
                        <StatusBadge status={strategy.latestRun.status} category="run"/>
                        <p className="text-xs text-muted-foreground mt-1">
                            {formatRelativeTime(strategy.latestRun.startedAt)}
                        </p>
                    </div>) : (<span className="text-xs text-muted-foreground hidden sm:block">never run</span>)}
                <Button size="sm" variant="outline" disabled={!strategy.enabled || isRunning} onClick={(e) => {
            e.stopPropagation();
            onRun();
        }} className="h-8">
...
export default function StrategiesPage() {
    ...
    if (groupedByVenue.length === 0) {
        return (<div className="space-y-6">
                ...
                <EmptyState icon={Layers} title="No strategies" description="No strategies configured yet"/>
            </div>);
    }
    return (<div className="space-y-6">
            <div className="flex justify-end">
            ...
            {groupedByVenue.map(({ app, meta, strategies }) => (<div key={app} className="space-y-3">
                    ...
                    <div className="space-y-2">
                        {strategies.map((strategy) => (<StrategyCard key={strategy._id} strategy={strategy} onRun={() => {
