    ...
    return (<div className="space-y-6">
            <div className="space-y-3">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    {run ? (<div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:flex-wrap">
                            <div className="flex items-center gap-2 flex-wrap">
                                <VenueBadge app={run.app}/>
                                <StatusBadge status={run.status} category="run"/>
                ...
                : ""}
                            ...
                            <div className="flex items-center gap-1.5 sm:ml-auto">
                                {run.status === "running" ? (<Button variant="outline" size="sm" onClick={handleStop} disabled={stopping}>
                                        {stopping
            ...
            {run?.error ? (<Card className="border-signal-danger/30">
                    <CardHeader className="pb-2">
            ...
            <Card>
                ...
                <CardContent>
                    {agentLogs.length === 0 ? (<EmptyState icon={MessageSquare} title="No agent logs" description="No reasoning trace recorded for this run"/>) : (<div className="space-y-3 max-h-[60vh] sm:max-h-[600px] overflow-y-auto">
                            {agentLogs.map((log) => (<div key={log._id} className="rounded-md border border-border-subtle p-3">
                                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                                        <Badge variant="outline" className="text-xs">
                                            {log.role}
                                        </Badge>
                                        {log.toolName ? (<Badge variant="secondary" className="text-xs font-mono">
                                                {log.toolName}
                                            </Badge>) : null}
                                    ...
                                    {log.role === "assistant" && log.content ? (<MarkdownContent content={log.content}/>) : (<pre className="text-xs whitespace-pre-wrap break-words font-mono bg-muted/50 rounded p-2 max-h-[200px] max-w-full overflow-auto">
                                            {log.content}
                                        </pre>)}
                                    {log.toolInput ? (<details className="mt-2">
                                    ...
                                    {log.toolOutput ? (<details className="mt-2">
                                            <summary className="text-xs text-muted-foreground cursor-pointer">
                                                Tool Output
                                            </summary>
            ...
            <Card>
                ...
                <CardContent>
                    {tradeEvents.length === 0 ? (<EmptyState icon={ArrowRightLeft} title="No trade events" description="No order intents or executions for this run"/>) : (<div className="space-y-2">
                ...
                return (<div key={event._id} className="flex flex-col gap-2 sm:flex-row sm:items-start sm:gap-3 rounded-md border border-border-subtle p-3">
                                        <StatusBadge status={event.eventType} category="event" className="text-xs shrink-0 w-fit"/>
                                        <div className="min-w-0 flex-1">
                                            <p className="text-xs text-muted-foreground">
                                                {formatTimestamp(event.timestamp)}
                                            </p>
                                            {executionError ? (<div className="mt-2 rounded border border-signal-danger/30 bg-signal-danger/5 p-2 text-xs text-signal-danger">
                                                    <div className="flex flex-wrap items-center gap-2">
                                                        {executionError.source ? (<Badge variant="outline" className="text-[10px] uppercase tracking-wide">
                                                                {executionError.source}
                                                            </Badge>) : null}
                                                        {executionError.code ? (<Badge variant="outline" className="text-[10px] font-mono">
                                                                {executionError.code}
                                                            </Badge>) : null}
                                                    </div>
                                            ...
                                            {payload ? (<pre className="text-xs font-mono mt-1 bg-muted/50 rounded p-2 overflow-auto max-h-[150px] break-words whitespace-pre-wrap">
...
function extractExecutionError(payload) {
    if (!payload) {
        return null;
    }
