...
import { VENUE_META, ACTIVE_VENUE_APPS } from "@/lib/constants";
...
export default function KillSwitchesPage() {
    ...
    const handleToggle = (scope, enabled) => {
    ...
    return (<div className="space-y-6">
            <Card className={systemState.globalKillSwitch ? "border-signal-danger/30" : ""}>
                <CardHeader>
                    <div className="flex items-center justify-between gap-3">
                        ...
                        <Switch className="shrink-0" checked={systemState.globalKillSwitch} onCheckedChange={(checked) => handleToggle("global", checked)}/>
                    </div>
                </CardHeader>
                {systemState.updatedAt > 0 ? (<CardContent>
            ...
            const key = app.replace(/-/g, "_");
            const isKilled = systemState.appKillSwitches[key] === true;
            return (<Card key={app} className={isKilled ? "border-signal-danger/30" : ""}>
                            <CardContent className="flex items-center justify-between gap-3 py-4">
                                ...
                                <Switch className="shrink-0" checked={isKilled} onCheckedChange={(checked) => handleToggle(app, checked)}/>
                            </CardContent>
                        </Card>);
        })}
            </div>
