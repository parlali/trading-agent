...
export default function HealthPage() {
    ...
    if (health.length === 0) {
        return (<EmptyState icon={Heart} title="No heartbeats" description="No apps have reported health status yet"/>);
    }
    return (<div className="space-y-3">
            {health.map((hb) => {
            const isStale = isHeartbeatStale(hb.lastHeartbeat);
            const effectiveStatus = isStale ? "unhealthy" : hb.status;
            return (<Card key={hb._id}>
                        <CardContent className="flex items-center justify-between py-4">
                            ...
                            <div className="flex items-center gap-2">
                                {isStale ? (<Badge variant="destructive" className="text-xs">stale</Badge>) : null}
                                <StatusBadge status={effectiveStatus} category="health" fallback="destructive"/>
                            </div>
                        </CardContent>
                        {hb.metadata ? (<CardContent className="pt-0">
