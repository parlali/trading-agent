...
import { EquityChart, FreshnessHeader, PortfolioSection, ProviderFilter, TIME_RANGES, } from "@/components/portfolio";
...
export default function EquityPage() {
    ...
    return (<div className="space-y-6">
            ...
            : "Combined provider-truth equity across all venue accounts"}
                ...
                <FilterBar items={TIME_RANGES.map((range) => range.value)} selected={timeRange} onSelect={setTimeRange} getLabel={(value) => TIME_RANGES.find((range) => range.value === value).label} variant="tabs"/>
            </div>

            {equityData.latest ? (<div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
                    <StatCard label={provider ? `${VENUE_META[provider].shortLabel} Equity` : "Total Equity"} value={latestTotal} format="currency" size="2xl"/>
                    <StatCard label="Period Change" value={periodChange} format="pnl" size="2xl"/>
                    {providerStats.map((appData) => {
                const meta = VENUE_META[appData.app];
                return (<StatCard key={appData.app} label={meta.shortLabel} value={appData.latest} format="currency" size="lg">
