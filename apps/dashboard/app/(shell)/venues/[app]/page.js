...
export default function VenuePage({ params, }) {
    ...
    const killSwitchKey = app.replace(/-/g, "_");
    ...
    return (<div className="space-y-6">
            <div className="flex items-center gap-3">
                ...
                {heartbeat ? (<div className="ml-auto flex items-center gap-2">
                        <StatusDot status={heartbeat.status}/>
                        <span className="text-sm text-muted-foreground">{heartbeat.status}</span>
                    </div>) : null}
            </div>

            {isKilled ? (<div className="flex items-center gap-2 rounded-lg border border-signal-danger/30 bg-signal-danger/10 px-4 py-3">
                    <span className="text-sm font-medium text-signal-danger">
                        Kill switch active for {meta?.shortLabel ?? app}
                    </span>
                </div>) : null}

            {snapshot ? (<div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                <Card>
                    ...
                    <CardContent>
                        {venueStrategies.length === 0 ? (<EmptyState icon={Layers} title="No strategies" description={`No strategies configured for ${meta?.shortLabel ?? app}`}/>) : (<div className="space-y-2">
                                {venueStrategies.map((strategy) => (<div key={strategy._id} className="flex items-center justify-between rounded-lg border border-border-subtle p-3">
                                        <div className="flex items-center gap-2">
                                            <StatusDot status={strategy.enabled ? "healthy" : "unhealthy"}/>
                ...
                <Card>
                    ...
                    <CardContent>
                        {venueRuns.length === 0 ? (<EmptyState icon={Play} title="No runs" description="No recent runs for this venue"/>) : (<div className="space-y-2">
                                {venueRuns.map((run) => {
                const strategy = venueStrategies.find((s) => String(s._id) === String(run.strategyId));
                return (<div key={run._id} className="flex items-center justify-between text-sm">
                                            <div className="flex items-center gap-2">
                    ...
                    <CardContent>
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {venuePositions.map((pos, i) => (<tr key={i} className="border-b border-border-subtle last:border-0">
