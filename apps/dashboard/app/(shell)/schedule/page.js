...
function TriggerBadge({ trigger }) {
    switch (trigger) {
        case "callback":
            return (<Badge variant="outline" className="text-xs gap-1 border-chart-2/30 text-chart-2">
                    <Zap className="h-3 w-3"/>
                    agent
                </Badge>);
        case "manual":
            return (<Badge variant="outline" className="text-xs gap-1">
                    manual
                </Badge>);
        default:
            return (<Badge variant="outline" className="text-xs gap-1 border-muted-foreground/30">
                    <Clock className="h-3 w-3"/>
                    cron
                </Badge>);
    }
}
function NextFireInfo({ entry }) {
    ...
    if (callback && callback.firesAt > Date.now()) {
        ...
        return (<div className="space-y-1">
                ...
                {nextCronCountdown ? (<div className="flex items-center gap-1.5">
...
function ScheduleRow({ entry }) {
    return (<Link href={`/strategies/${entry._id}`} className="flex items-center justify-between rounded-lg border border-border-subtle p-3 sm:p-4 transition-colors hover:bg-muted/50 hover:border-border group">
            <div className="flex items-center gap-3 min-w-0 flex-1">
                <StatusDot status={entry.isRunning
            ? "running"
            : entry.enabled
                ? "healthy"
                : "unhealthy"}/>
                ...
                <div className="text-right min-w-[90px]">
                    {entry.latestRun ? (<div>
                            <div className="flex items-center gap-1.5 justify-end">
                                <StatusBadge status={entry.latestRun.status} category="run"/>
...
export default function SchedulePage() {
    ...
    if (data.length === 0) {
        return (<EmptyState icon={Calendar} title="No strategies" description="Create a strategy to see its schedule here"/>);
    }
    const entries = data;
    const sorted = [...entries].sort((a, b) => {
        ...
        if (aNextMs !== null && bNextMs !== null)
            return aNextMs - bNextMs;
        if (aNextMs !== null)
            return -1;
        if (bNextMs !== null)
            return 1;
    ...
    const withCallback = sorted.filter((e) => !e.isRunning && e.pendingCallback && e.pendingCallback.firesAt > Date.now());
    const enabled = sorted.filter((e) => !e.isRunning && e.enabled && !(e.pendingCallback && e.pendingCallback.firesAt > Date.now()));
    ...
    return (<div className="space-y-6">
            {running.length > 0 ? (<Card>
                    ...
                    <CardContent className="space-y-2">
                        {running.map((entry) => (<ScheduleRow key={entry._id} entry={entry}/>))}
                    </CardContent>
                </Card>) : null}

            {withCallback.length > 0 ? (<Card>
                    <CardHeader className="pb-3">
                        <CardTitle className="text-base flex items-center gap-2">
                    ...
                    <CardContent className="space-y-2">
                        {withCallback.map((entry) => (<ScheduleRow key={entry._id} entry={entry}/>))}
                    </CardContent>
                </Card>) : null}
            ...
            <Card>
                ...
                <CardContent className="space-y-2">
                    {enabled.length > 0 ? (enabled.map((entry) => (<ScheduleRow key={entry._id} entry={entry}/>))) : (<p className="text-sm text-muted-foreground py-2">No active schedules</p>)}
                </CardContent>
            </Card>

            {disabled.length > 0 ? (<Card>
                    <CardHeader className="pb-3">
                        <CardTitle className="text-base flex items-center gap-2">
                            Disabled
                            <Badge variant="secondary" className="text-xs">
                    ...
                    <CardContent className="space-y-2">
                        {disabled.map((entry) => (<ScheduleRow key={entry._id} entry={entry}/>))}
                    </CardContent>
                </Card>) : null}
        </div>);
}
