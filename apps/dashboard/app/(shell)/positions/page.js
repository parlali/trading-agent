...
import { DataTable, CardList, PortfolioSection, ProviderFilter, FreshnessHeader, SideBadge, EquityChart, TradeHistoryTable, TIME_RANGES, } from "@/components/portfolio";
...
const positionColumns = [
    ...
    },
    {
        ...
        render: (pos) => (<span className={pos.stopLoss !== undefined ? "text-loss" : "text-muted-foreground"}>
    ...
    },
    {
        ...
        align: "right",
        cellClassName: "font-mono tabular-nums text-xs",
        render: (pos) => (<span className={pos.takeProfit !== undefined ? "text-profit" : "text-muted-foreground"}>
...
function PositionsTab({ positions }) {
    ...
    if (positions.length === 0) {
        return (<EmptyState icon={Activity} title="No open positions" description="Open positions across all venues will appear here"/>);
    }
    return (<div className="space-y-3">
            ...
            <div className="sm:hidden">
                <CardList data={positions} getKey={getPositionKey} renderCard={(pos) => (<div className="rounded-lg border border-border-subtle p-3 space-y-2">
            ...
            <PortfolioSection className="hidden sm:block">
                <DataTable columns={positionColumns} data={positions} getRowKey={getPositionKey}/>
            </PortfolioSection>
        </div>);
}
function PendingOrdersTab({ orders }) {
    if (orders.length === 0) {
        return (<EmptyState icon={ClipboardList} title="No pending orders" description="Working orders across all venues will appear here"/>);
    }
    return (<div className="space-y-3">
            <p className="text-sm text-muted-foreground">
            ...
            <div className="sm:hidden">
                <CardList data={orders} getKey={(o) => o.orderId} renderCard={(order) => (<div className="rounded-lg border border-border-subtle p-3 space-y-2">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2 min-w-0">
                                    <span className="text-sm font-medium font-mono truncate">
                                        {order.instrument}
                            ...
                            <div className="flex items-center justify-between text-xs text-muted-foreground">
                                ...
                                <div className="flex items-center gap-3 font-mono tabular-nums shrink-0">
                                    ...
                                    {order.limitPrice != null ? (<span>@ {formatCurrency(order.limitPrice)}</span>) : null}
                                </div>
                            </div>
                        </div>)}/>
            </div>
            ...
            <PortfolioSection className="hidden sm:block">
                <DataTable columns={pendingOrderColumns} data={orders} getRowKey={(o) => o.orderId}/>
            </PortfolioSection>
        </div>);
}
...
function EquityTab({ provider, }) {
    ...
    const chartData = useMemo(() => {
        if (!equityData || equityData.series.length === 0)
            return [];
    ...
    return (<div className="space-y-3">
            <div className="flex items-center justify-between">
                ...
                <FilterBar items={TIME_RANGES.map((r) => r.value)} selected={timeRange} onSelect={setTimeRange} getLabel={(v) => TIME_RANGES.find((r) => r.value === v).label} variant="tabs"/>
            </div>

            {equityData.latest ? (<div className="flex items-center gap-4 text-sm">
    ...
    return (<div className="space-y-4">
            ...
            <Tabs defaultValue="positions">
                ...
                <TabsContent value="orders">
                    <PendingOrdersTab orders={pendingOrders}/>
                </TabsContent>
